...
public:
    ...
    static const size_t npos = -1; // вместо создания переменной в каждом обьекте, static выделяет память для переменной и ею могут пользоваться все обьекты.
    //static const size_t npos = UINTMAX_MAX; в данном случае -1 выставляет все значения байта в 1, т.е мы получаем максимальное значение
